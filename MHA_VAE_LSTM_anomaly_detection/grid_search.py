...
param_grid = {
    ...
    "l_win": [200, 400, 600, 800, 1000, 1500, 1800, 2000],
    "look_back_num": [1, 2, 3, 4, 5],
    "threshold_percent": [60, 70, 80, 90]
}

# Create directory for grid search configurations
grid_search_dir = Path("grid_search_configs_0414-nosein_noseout9-final_winlen_validation-MHA_VAE_LSTM")
...
# Open results summary file in append mode to continue recording results
...
def run_experiment(config, config_id):
    ...
    try:
        ...
        train_cmd = f"python mha_vae_lstm_main.py -c {config_path}"
        ...
        result_dir = os.path.join(save_dir, f"{config['exp_name']}-{config['dataset']}-win_len{config['l_win']}-look_back{config['look_back_num']}-batchsize{config['batch_size']}-VAE_units{config['num_hidden_units_vae']}-codesize{config['code_size']}-LSTM_batchsize{config['lstm_batch_size']}-layers{config['num_layers']}-units{config['num_hidden_units_lstm']}", "result/")
        
        # Extract detection rate (recall)
        detection_rate = -1
        try:
...
def main():
    ...
    with open('mha_vae_lstm_config.json', 'r') as f:
    ...
    # Get the latest completed experiment ID from results file
    latest_id = 0
    try:
        with open(results_file, 'r') as f:
            ...
            for line in f:
                if line.strip():  # Skip empty lines
                    latest_id = int(line.split(',')[0])
        print(f"Latest completed experiment ID: {latest_id}")
    except FileNotFoundError:
        latest_id = 0
        print(f"No completed experiments found. Starting from ID 0.")
    time.sleep(5)
