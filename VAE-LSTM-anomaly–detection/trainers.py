...
class VAETrainer:
    def __init__(self, model, train_loader, val_loader, optimizer, config, device, mean, std):
        ...
        self.data_std = std
        self.epoch = 0
        self.patience = config.get('vae_patience', 40)
    ...
    def find_anomalies(self, test_loader, lstm_model):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_loader):  # batch_size should be 1
                ...
                ''' reconstruction error calculation: point by point, sum across all modalities for all points, resulting in a single value for the whole batch window '''
                reconstruction_error = torch.sum(((batch - reconstructed) ** 2).squeeze((0, -1))).cpu().numpy()
                
                reconstruction_errors.append(reconstruction_error)
            
            '''
            reconstruction_errors -> batch by batch, single value records
            '''
            ...
            for batch in test_loader:   # batch_size should be 1
                ...
                embed_mean = mean.squeeze()  
                embed_mean = embed_mean.permute(1, 0)
            ...
            for i in range(0, len(x_lstm)): # step should be 1
                ...
                prediction_error = torch.sum(((decode_targets_recon - decode_outputs_recon) ** 2).squeeze((0, -1))).detach().cpu().numpy()
            ...
            # ATTENTION: The first batch in the test_loader is not included in the anomaly_scores list (due to the lstm_look_back_num) -> add 1 to the whole list
            predicted_anomalies_window_idx = np.array([i for i, score in enumerate(anomaly_scores) if score >= anomaly_threshold]) + 1
    ...
    def visualize_anomalies(self, full_data_used, adj_true_anomaly_intervals, predicted_anomalies_window_idx, detect_rate, precision=None, f_beta=None):
        ...
        for i in range(full_data_used.shape[1]):
            plt.plot(full_data_used[:, i], label=f"Full_data_modal_{i+1}", alpha=0.8, color=color_list[i % len(color_list)])
        ...
        title = f"{exp_dataset} - VAE-LSTM - Anomaly Detection Results\n(Recall: {detect_rate:.4f}"
...
class LSTMTrainer:
    def __init__(self, model, lstm_train_embeddings, lstm_val_embeddings, optimizer, config, device, mean, std, vae_model):
        ...
        self.patience = config.get('lstm_patience', 200)
        self.best_val_loss = float('inf')
        self.patience_counter = 0
    ...
    def create_lstm_sequence(self):   # create lstm input & prediction target sequences
        ...
        def _create_embedding_sequence(embeddings):
            ...
            for i in range(len(embeddings) - self.look_back_num - 1):
                x.append(embeddings[i : i + self.look_back_num])    
                y.append(embeddings[i + self.look_back_num])        
