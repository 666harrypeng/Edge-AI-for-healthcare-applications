...
class VAETrainer:
    ...
    def generate_lstm_embeddings(self, vae_model, train_loader, val_loader):
        ...
        def _generate_embeddings(data_loader):  # prefer -> use non-reparameterization -> less stochastic, easier to train LSTM
            embeddings = []
            with torch.no_grad():
    ...
    def find_anomalies(self, test_loader, lstm_model):
        ...
        with torch.no_grad():
            ...
            '''
            # dispose the first <lstm_look_back_num> windows to align (those windows are used for prior learning for LSTM)
            ...
            print(f"number of predicted anomalies: {len(predicted_anomalies_window_idx)} (raw anomaly window number: {raw_predicted_anomalies_window_count}; prediction error deviation ratio threshold: {prediction_error_deviation_ratio_threshold})")
