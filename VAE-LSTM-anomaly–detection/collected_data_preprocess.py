...
def preprocess_and_save(csv_folder_path, dataset, data_dir, config):
    ...
    full_rolling_anomaly_idx = [idx for idx in anomalies if idx <= (window_num * window_size)]    
    ...
    print(f"Number of anomaly points in validation set: 0 (only to validate prediction training, no anomalies in validation set)")  
...
def main():
    ...
    parser.add_argument('-c', '--config', default='./VAE-LSTM_config.json', help='Path to the configuration JSON file')
