...
def preprocess_and_save(csv_folder_path, dataset, save_dir_path, val_ratio=0.1, split_ratio=0.9, save_file=True, window_size=48, json_file='./config.json'):
    # Load raw data
    ...
    '''full_rolling contains anomalies'''
    full_rolling = create_rolling_windows(readings_normalized, window_size)
    window_num = len(readings_normalized) // window_size
    full_rolling_anomaly_idx = [idx for idx in idx_anomaly if idx <= (window_num * window_size)]    # dispose anomalies in the remaining data points that cannot be constructed into one window
    ...
    if save_file:
        ...
        np.savez(save_path, 
                 t=t, 
                 readings=readings,  # Original readings (unmodified)
...
def main():
    ...
    for dataset in datasets:
        preprocess_and_save(csv_folder_path, dataset, save_dir_path, window_size=config['l_win'], json_file='./pure_lstm_config.json')
