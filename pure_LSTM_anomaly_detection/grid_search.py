...
grid_search_dir = Path("grid_search_configs_0414-nosein_noseout9-final_winlen_validation-pure_LSTM")
...
def run_experiment(config, config_id):
    ...
    try:
        ...
        train_cmd = f"python pure_lstm_anomaly_detection_main.py -c {config_path}"
        ...
        result_dir = os.path.join(save_dir, f"{config['exp_name']}-{config['dataset']}-win_len{config['l_win']}-look_back{config['look_back_num']}-LSTMunits{config['num_hidden_units_lstm']}-LSTMlayers{config['num_layers']}", "result/")
...
def main():
    ...
    with open('pure_lstm_config.json', 'r') as f:
