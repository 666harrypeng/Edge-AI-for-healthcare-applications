...
#define OXYGEN_CHUNK_SIZE 100 // data length for blood oxygen
int32_t spo2; //SPO2 value
int32_t last_spo2; // last spo2 value (for data recording continuity)
int8_t validSPO2; //indicator to show if the SPO2 calculation is valid
int32_t heartRate; //heart rate value
int8_t validHeartRate; //indicator to show if the heart rate calculation is valid
uint32_t irBuffer[OXYGEN_CHUNK_SIZE]; //infrared LED sensor data
uint32_t redBuffer[OXYGEN_CHUNK_SIZE];  //red LED sensor data
double avered = 0.0;
double aveir = 0.0;
double sumirrms = 0.0;
...
void get_blood_oxygen_values()
{
    ...
    for (byte i = 25; i < OXYGEN_CHUNK_SIZE; i++)
    {
        redBuffer[i - 25] = redBuffer[i];
        irBuffer[i - 25] = irBuffer[i];
    }
    // Serial.println("get_blood_oxygen_values: dumping the first 25 sets of samples in the memory and shift the last 75 sets of samples to the top");

    //take 25 sets of samples before calculating the heart rate.
    for (byte i = 75; i < OXYGEN_CHUNK_SIZE; i++)
    {
        while (particleSensor.available() == false) //do we have new data?
        {
            particleSensor.check(); //Check the sensor for new data
            // Serial.println("get_blood_oxygen_values: particleSensor.check()");

        }

        redBuffer[i] = particleSensor.getRed();
        // Serial.println("get_blood_oxygen_values: particleSensor.getRed()");
        irBuffer[i] = particleSensor.getIR();
        // Serial.println("get_blood_oxygen_values: particleSensor.getIR()");
        particleSensor.nextSample(); //We're finished with this sample so move to next sample
        
        // Serial.println("get_blood_oxygen_values: finished next sample");
    ...
    // Serial.println("get_blood_oxygen_values: finished maxim_heart_rate_and_oxygen_saturation");
    
    // If spo2 is out of range -> set to last_spo2
    if (spo2 < 0 || validSPO2 != 1)
    {
        spo2 = last_spo2;
    }
    else // if spo2 is in range -> update last_spo2
    {
        last_spo2 = spo2;
    }
    ...
    // Serial.println("get_blood_oxygen_values: finished updating spo2");
}
...
void loop()
{
	...
	if (currentTime - lastSampleTime >= SAMPLE_INTERVAL_MS)
	{
		...
		{	
			...
			// Serial.println("get_blood_oxygen_values");
		...
		if (TXMsg.samples >= BUFFER_SIZE)
		{
			...
			// Debug print
			Serial.print("A[0]: " + String(TXMsg.audio_buffer[0]));
